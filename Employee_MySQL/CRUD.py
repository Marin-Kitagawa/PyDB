...
class Employee:
    ...
    def __init__(self, name, salary, dob, doj, department, position, email, phone):
        ...
        self.cursor.execute(
            f"""
            INSERT INTO emp(id, name, salary, dob, doj, department, position, email, phone)
            VALUES(
                '{self.id}', 
                '{name}', 
                '{salary}', 
    ...
    def get_employee_by_id(self, id):
        ...
        self.cursor.execute(f"SELECT * FROM emp WHERE id = '{id}';")
    ...
    def get_employee_id_by_phone(self, phone):
        for i in self.database:
            if self.database[i]['phone'] == phone:
                pprint(i)
        self.cursor.execute(f"SELECT id FROM emp WHERE phone = '{phone}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employee_id_by_email(self, email):
        for i in self.database:
            if self.database[i]['email'] == email:
                pprint(i)
        self.cursor.execute(f"SELECT id FROM emp WHERE email = '{email}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employees_by_name(self, name):
        ...
        if len(found) == 0:
            ...
            for i in found:
                pprint(i)
        self.cursor.execute(f"SELECT * FROM emp WHERE name = '{name}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employee_by_phone(self, phone):
        ...
        if not found:
            ...
            pprint(found)
        self.cursor.execute(f"SELECT * FROM emp WHERE phone = '{phone}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employees_by_department(self, department):
        ...
        if len(found) == 0:
            ...
            for i in found:
                pprint(i)
        self.cursor.execute(f"SELECT * FROM emp WHERE department = '{department}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employees_by_position(self, position):
        ...
        if len(found) == 0:
            ...
            for i in found:
                pprint(i)
        self.cursor.execute(f"SELECT * FROM emp WHERE position = '{position}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employees_by_doj(self, doj):
        ...
        if len(found) == 0:
            ...
            for i in found:
                pprint(i)
        self.cursor.execute(f"SELECT * FROM emp WHERE doj = '{doj}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def get_employee_by_email(self, email):
        ...
        self.cursor.execute(f"SELECT * FROM emp WHERE email = '{email}';")
        for result in self.cursor.fetchall():
            pprint(result)
    ...
    def update_employee_name(self, name, new_name):
        ...
        if len(found) == 0:
            ...
            self.cursor.execute(f"UPDATE emp SET name = '{new_name}' WHERE name = '{name}';")
        else:
            it = 0
            ...
            while choice not in range(1, len(found) + 1):
                ...
                choice = int(input())
            self.database[ids[choice - 1]]['name'] = new_name
            self.cursor.execute(f"UPDATE emp SET name = '{new_name}' WHERE id = '{ids[choice - 1]}';")
    ...
    def update_employee_phone(self, phone, new_phone):
        ...
        for i in self.database:
            if self.database[i]['phone'] == phone:
                ...
                self.cursor.execute(f"UPDATE emp SET phone = '{new_phone}' WHERE phone = '{phone}';")
                found = True
    ...
    def update_employee_salary_by_email(self, email, salary):
        ...
        for i in self.database:
            if self.database[i]['email'] == email:
                ...
                self.cursor.execute(f"UPDATE emp SET salary = '{salary}' WHERE email = '{email}';")
                found = True
    ...
    def update_employee_salary_by_phone(self, phone, salary):
        ...
        for i in self.database:
            if self.database[i]['phone'] == phone:
                self.database[i]['salary'] = salary
                self.cursor.execute(f"UPDATE emp SET salary = '{salary}' WHERE phone = '{phone}';")
                found = True
    ...
    def update_employee_department_by_email(self, email, new_department):
        ...
        for i in self.database:
            if self.database[i]['email'] == email:
                ...
                self.cursor.execute(f"UPDATE emp SET department = '{new_department}' WHERE email = '{email}';")
                found = True
    ...
    def update_employee_department_by_phone(self, phone, new_department):
        ...
        for i in self.database:
            if self.database[i]['phone'] == phone:
                self.database[i]['department'] = new_department
                self.cursor.execute(f"UPDATE emp SET department = '{new_department}' WHERE phone = '{phone}';")
                found = True
                pprint(f"Department has been changed for user with ID: {i}")
        if not found:
            pprint("No employee with the given phone number exists")
    ...
    def update_employee_position_by_email(self, email, new_position):
        ...
        for i in self.database:
            if self.database[i]['email'] == email:
                ...
                self.cursor.execute(f"UPDATE emp SET position = '{new_position}' WHERE email = '{email}';")
                found = True
    ...
    def update_employee_position_by_phone(self, phone, new_position):
        ...
        for i in self.database:
            if self.database[i]['phone'] == phone:
                self.database[i]['position'] = new_position
                self.cursor.execute(f"UPDATE emp SET position = '{new_position}' WHERE phone = '{phone}';")
                found = True
                pprint(f"Position has been changed for user with ID: {i}")
        if not found:
    ...
    def update_employee_email(self, email, new_email):
        ...
        for i in self.database:
            if self.database[i]['email'] == email:
                ...
                self.cursor.execute(f"UPDATE emp SET email = '{new_email}' WHERE email = '{email}';")
                found = True
    ...
    def delete_employee_by_id(self, id):
        if id in self.database:
            ...
            self.cursor.execute(f"DELETE FROM emp WHERE id = '{id}';")
        else:
            pprint("ID NOT FOUND\n")
    ...
    def delete_employees_by_department(self, department):
        ...
        if len(employees) == 0:
            ...
            self.cursor.execute(f"DELETE FROM emp WHERE department = '{department}';")
        else:
            ...
            if choice.lower() == 'yes':
                ...
                for i in ids:
                    if i in self.database:
                        del self.database[i]
                        self.cursor.execute(f"DELETE FROM emp WHERE department = '{department}' AND id = '{i}';")
    ...
    def delete_employees_by_position(self, position):
        ...
        if len(employees) == 0:
            ...
            del self.database[employees[0]]
            self.cursor.execute(f"DELETE FROM emp WHERE id = '{employees[0]}';")
        else:
            ...
            if choice.lower() == 'yes':
                for i in employees:
                    del self.database[i]
                    self.cursor.execute(f"DELETE FROM emp WHERE id = '{i}';")
            else:
                ids = input("Enter the ids of the employees to be deleted separated by space\n\n").split()
                for i in ids:
                    if i in self.database:
